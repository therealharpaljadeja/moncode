...
function AppPage() {
  ...
  const projectApi = useCallback(
    (path: string) => `/api/projects/${projectId}${path}`,
    [projectId],
  );
  ...
  // True once POST /api/projects/[id]/sandbox has responded and the in-memory
  // session is registered, so the boot SSE stream can connect.
  ...
  const ensureSandbox = useCallback(async () => {
    ...
    if (!token) {
      ...
      setBootError("Not authenticated. Try signing out and back in.");
    ...
    setBootError(null);
    try {
      const res = await authFetch(projectApi("/sandbox"), { method: "POST" });
      ...
      if (!res.ok) {
        setBootStatus("failed");
        setBootError(data.error ?? `Sandbox request failed (${res.status})`);
        return;
      }
  ...
  }, [authFetch, getAccessToken, projectApi]);

  const generateTitle = useCallback(
    async (prompt: string) => {
      try {
        const res = await authFetch(projectApi("/title"), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ prompt }),
        });
        if (!res.ok) {
          console.warn("Title generation failed:", res.status);
          return;
        }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        await consumeSse(
          projectApi("/sandbox/stream"),
          {
            phase: (data) => {
              const phase = data as { key?: string; label?: string };
              if (
                phase &&
                typeof phase.key === "string" &&
                typeof phase.label === "string"
              ) {
                setBootPhase({ key: phase.key, label: phase.label });
              }
            },
            status: (data) => {
              ...
              if (status.status === "ready") {
                setBootStatus("ready");
                setSandboxUrl(status.sandboxUrl ?? null);
              } else if (status.status === "failed") {
                setBootStatus("failed");
                setBootError(status.bootError ?? "boot failed");
              }
            },
          },
        ...
        setBootStatus((current) => {
          if (current !== "pending") return current;
          return "failed";
        });
        setBootError((current) => {
          if (current) return current;
          return err instanceof Error ? err.message : "Boot stream failed";
        });
      }
    })();

    return () => abort.abort();
  }, [privyReady, postReturned, bootStatus, projectApi, getAccessToken]);

  const refetchFiles = useCallback(async () => {
    try {
      const res = await authFetch(projectApi("/files"));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await authFetch(projectApi("/transcript"));
        if (!res.ok) return;
        ...
        const firstPrompt = extractFirstUserPromptFromMessages(messages);
        if (!cancelled && firstPrompt) {
          void generateTitle(firstPrompt);
        }
      } catch {
  ...
  }, [bootStatus, transcriptLoaded, authFetch, projectApi, generateTitle]);
  ...
  const sendPrompt = useCallback(
    async (text: string) => {
      ...
      if (!title && !animatingTitle) {
        void generateTitle(trimmed);
      }
      setBusy(true);

      try {
        const res = await authFetch(projectApi("/chat"), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    [refetchFiles, title, animatingTitle, generateTitle, authFetch, projectApi],
  );
    ...
    >
      ...
      <ResizablePanel defaultSize={60} minSize={30} className="min-w-0">
        <RightPane
          ...
          onRetryBoot={() => void ensureSandbox()}
          tab={tab}
          setTab={setTab}
          tree={tree}
...
function extractFirstUserPromptFromMessages(
  ...
  if (bootStatus !== "ready") {
    return (
      <div className="flex h-full min-h-0 flex-col">
        ...
        {bootStatus === "failed" && (
          <div className="flex shrink-0 justify-center gap-2 border-t px-4 py-3">
